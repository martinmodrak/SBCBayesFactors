...
- Notationally distinguish model
parameters vs BMA parameters space 
- Ranef correct figures 
- Cleanup + figures for post t-test 
- Caching + bridgesampling - More simulations for ranef

Important: 
- Schad bridgesampling 2x2 (claimed miscalib, actually OK) 
- Find a failure mode for bridgesamplinb/lmbf (turtles?!) 
- Gaffke history 
...
The posterior distribution $\pi_\text{BMA}(i | y)$ of $i$ is then fully determined by the
...
Due to the tight connection between Bayes factors, marginal data distributions and BMA,
...
But when using SBC instead of just binary calibration, we can extend the check --- we can
test the correctness of the full BMA model.
...
Typically not full joint distribution but univariate projections
$f: \Theta \times Y \to \mathbb{R}$, although the literature so far only discusses
data-independent test quantities. Similarly to SBC, we will also say that $\phi$ has the
correct data-averaged posterior w.r.t. data-independent quantities.
...
-   **Flipped model indices**, i.e.
    $\forall i: \mathcal{M}^\prime_i = \mathcal{M}_{K - i - 1}$ - when $K = 2$ and
    $\text{Pr}(\mathcal{M}_0) = \text{Pr}(\mathcal{M}_1)$, this corresponds to computing
    the inverse of the Bayes factor
...
Many packages use improper priors. Many priors are unrealistic -\> we want to check only
relevant parts of the model configuration space. Both can be overcome with *posterior SBC*
(https://users.aalto.fi/\~ave/casestudies/postsbc/postsbc.html).

Assuming the data space partitions as $Y = Y_1 \times Y_2$, then given a fixed
$y_1 \in Y_1$ we can construct a model $\bar\pi$ such that:
...
We can then apply SBC (or DAP) to $\bar\pi$ by fitting $\pi$ to $y_1$, taking those draws
as prior draws, simulating $y^s_2$ using those prior draws and fitting $\pi$ to the whole
datasets $(y_1,y^s_2)$.

We can slightly extend this even draw $y_1$ from an arbitrary distribution (but then need
to obtain the samples from $\pi_\text{post}(\theta | y_1)$ for each value of $y_1$)

To check computation for models with improper priors, we need to choose $y_1$ as the
smallest dataset size such that $\pi_\text{post}(\theta | y_1)$ is proper. To target check
to a specific dataset, we may take a suitable portion of the dataset as $y_1$.

To use posterior SBC with Bayes factors, we need to compute the implied prior for both
models, but also the new prior probability of models. Alternatively, we can tweak the
prior probability, to get 0.5 with the newly computed Bayes factor --- this potentially
makes better use of computation, but requires to use a fixed $y_1$.

### Other approaches

The [good check](https://osf.io/preprints/psyarxiv/59gj8) (cite). This shares some of the
big problems with data-averaged posterior --- ignores the mapping between BFs and data.
Additionally, both the variance and second raw moment of the Bayes factor in favor of the
false hypothesis can have very large or even undefined variance, which explains the slow
convergence. Even though large variance of BFs is not a problem for either SBC or
...
($\theta_0 = \theta_1 = \emptyset$) problem:
...
be specific the DAP criterion for this model is:
...
detected by miscalibration or DAP, but SBC will detect it with suitable derived
...
but generally missed by DAP, see @fig-pnb-noise.
...
There is however a case where DAP works better than the other options, that is when the
...
simulations. This is discovered relatively quickly by all of the options, but DAP requires
...
It may be tempting to use a fixed value of $\mu$ and $\sigma^2$ or draw them from some
surrogate distribution. In this case however, the assumptions of all of the consistency
criteria (except the good check) do not hold. And indeed, we see subtle violations of all
checks.
...
We test a model with the intercept and a single fixed effect against a model that also has
a random effect. We will be using the BayesFactor package.
...
Now choosing the overall intercept as fixed constant results in apparent miscalibration
...
miscalibration test with DAP and SBC for the model index following very quickly. However
this is a false positive. Employing posterior SBC to produce a proper prior for the mean
and intercept results in perfect calibration in all methods even after running 10 000
simulations (@fig-ranef-post, miscalibration p = `round(ranef_test$miscalibration_p))`,
95% CI for DAP difference from expected mean `r ranef_ci_diff`). So we can conclude that
the BF calculation in the BayesFactor package works well for this case.
...
Uses an improper Jeffrey's prior on variance
$\pi_\text{prior}\left(\sigma^2\right) = \frac{1}{\sigma^2}$.

However, when we use posterior SBC, the computed Bayes factors pass all checks to high
precision (using 40 000 simulations).

### Schad et al.

Unfortunately, their work has multiple problems making their conclusions suspect: they do
not simulate from the correct prior for multiple parameters (not just those with improper
priors) and they focus on DAP (which we know is weak), finally they also compare the wrong
null hypothesis (they simulate null where three coefficients are 0, but compute BF agaisnt
a hypothesis that a single coefficient is 0).

Another problem is using Bayesian t-tests for testing means, which can be invalid (most
notably, if the posterior probabilities are constant). Here we combine classical t-tests
(which work well enough in most cases) and the test of Gaffke 2005, which is more
...
Neglecting vectorization https://link.springer.com/article/10.1007/s41237-024-00232-7 9/21
neglected the normalizing constant - reuse the turtle models

## Discussion

While there are cases where DAP performs somewhat better than SBC/prediction calibration,
it also fails catastrophically in many examples. Using DAP for validation of Bayes factor
computation should thus be considered only as a supplement to SBC or prediction
calibration.

Our results also show that to rule out problems in BF computation, simulation studies need
to use a lot of simulations --- even quite strong miscalibration can require several
hundred simulations to be reliably uncovered.
...
is the biggest problem. We can usually run more simulations to sort flukes from real
...
Checking packages for Bayes factors is difficult as the implied priors are typically
non-standard and at best partially documented.
