---
title: "lmBF - random effects presence"
format: html
---
...
res_prior_H0 <- sampling(prior_model_H0, data = prior_data_stan, iter = 51000, warmup = 1000, thin = 10)
res_prior_H1 <- sampling(prior_model_H1, data = prior_data_stan, iter = 51000, warmup = 1000, thin = 10, control = list(adapt_delta = 0.99))
...
```{r}
sim_lmbf_ranef_posterior <- function(N, N_subjects, formula, df_prior, draws, rscaleFixed, rscaleRandom) {
...
set.seed(854230)
ds_lmbf_post_H0 <- generate_datasets(
  SBC_generator_function(
    sim_lmbf_fixed_posterior,
    formula = f_H0,
    N = N,
    N_subjects = N_subjects,
    df_prior = prior_df,
    draws = draws_prior_H0,
    future.globals = "sim_lmbf_ranef_data"
  ),
  n_sims = n_sims
)
set.seed(6895266)
ds_lmbf_post_H1 <- generate_datasets(
  SBC_generator_function(
    sim_lmbf_ranef_posterior,
    formula = f_H0,
    N = N,
    N_subjects = N_subjects,
    df_prior = prior_df,
    draws = draws_prior_H1,
    rscaleFixed = r_scale_fixed, 
    rscaleRandom = r_scale_random,
    future.globals = "sim_lmbf_ranef_data"
  ),
  n_sims = n_sims
)
...
res_post <- compute_SBC(ds_post, backend_post, keep_fits = FALSE,
                            cache_mode = "results",
                            cache_location = file.path(cache_dir, paste0("lmbf_ranef_post_presence_", N_for_filename, ".rds")))

plot_ecdf_diff(res_post, combine_variables = combine_lmBF_arrays)

metrics_post <- calibration_metrics(res_post, prob1_prior = prob1_prior)
print(metrics_post)

saveRDS(metrics_post,
  file.path(cache_dir, paste0("lmbf_ranef_post_presence_", N_for_filename, "_metrics.rds")))
...
hist_post_okada <- compute_bootstrapped_histories(bp_post, history_length = 500, n_histories = 100, step = 10, min_sim_id = 20,
                                            compute_okada_history, prior_prob1 = prob1_prior)

plot_log_p_histories(hist_post_okada, "Fractional Good check")
...
N_subjects <- 15
n_sims <- 7500
constant_alpha_sigma <- list(alpha = 0, sigma = 1)

set.seed(854624)
ds_lmbf_constant_H0 <- generate_datasets(
  SBC_generator_function(
    sim_lmbf_fixed,
    N = N,
    N_subjects = N_subjects,
    formula = f_H0,
    alpha_sigma_prior = constant_alpha_sigma,
    rscaleFixed = r_scale_fixed, 
    future.globals = "sim_lmbf_ranef_data"
  ),
  n_sims = n_sims
...
set.seed(2645244)
ds_lmbf_constant_H1 <- generate_datasets(
  SBC_generator_function(
    sim_lmbf_ranef,
    N = N,
    N_subjects = N_subjects,
...
res_constant <- compute_SBC(ds_constant, backend_constant, keep_fits = FALSE,
                            cache_mode = "results",
                            cache_location = file.path(cache_dir, paste0("lmbf_ranef_post_presence_constant_", N_for_filename, "_", n_sims, ".rds")))

plot_ecdf_diff(res_constant, combine_variables = combine_lmBF_arrays)

metrics_constant <- calibration_metrics(res_constant)
print(metrics_constant)

saveRDS(metrics_constant,
  file.path(cache_dir, paste0("lmbf_ranef_post_presence_constant_", N_for_filename, "_metrics.rds")))

```

```{r}
hist_length_constant <- 750
hist_step_constant <- 10
stats_filtered <- res_constant$stats %>% filter(variable %in% c("model", "Subject-1", "Subject-2")) |> 
  mutate(variable = gsub("Subject", "𝛾", variable))
hist_ranef_constant <- compute_bootstrapped_histories(stats_filtered, history_length = hist_length_constant, n_histories = 100,
                                                       step = hist_step_constant,
                                            compute_log_gamma_history)

bp_ranef_constant <- binary_probabilities_from_stats(res_constant$stats)


hist_ranef_constant_ttest <- compute_bootstrapped_histories(bp_ranef_constant, history_length = hist_length_constant, n_histories = 100, step = hist_step_constant,
                                            compute_ttest_history, min_sim_id = 10)



hist_ranef_constant_miscalib <- compute_bootstrapped_histories(bp_ranef_constant, history_length = hist_length_constant, n_histories = 100, step = hist_step_constant,
                                            compute_miscalibration_history)


hist_ranef_constant_okada <- compute_bootstrapped_histories(bp_ranef_constant, history_length = hist_length_constant, n_histories = 100, step = hist_step_constant,
                                            compute_okada_history)

plot_log_gamma_histories(hist_ranef_constant)
plot_log_p_histories(hist_ranef_constant_ttest, "ttest")
plot_log_p_histories(hist_ranef_constant_miscalib, "Miscalibration resampling")
plot_log_p_histories(hist_ranef_constant_okada, "Fractional Good check")

save_histories("ranef_constant",
               log_gamma = hist_ranef_constant,
               ttest = hist_ranef_constant_ttest,
               miscalibration = hist_ranef_constant_miscalib,
               okada = okada)
