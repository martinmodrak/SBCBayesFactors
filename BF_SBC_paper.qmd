...
Any candidate posterior can be described just by two numbers $\phi_\text{BMA}(i | Y = 1) = b_i$. Analytically, we can show that $\phi_\text{BMA}$ will satisfy the data-averaged posterior criterion if and only if $b_0 + b_1 = 1$, i.e. allowing infinitely many wrong Bayes factors, including assigning 100% probability to the wrong model or having $\phi_\text{BMA}(i | Y = 1) = \pi_\text{BMA}( 1 - i | Y = 1)$. To be specific the DAP criterion for this model is:

TODO 

$$
\forall i \in \{0,1\}: \text{Pr}(\mathcal{M}_i) = \sum_{y=0}^1  \phi_\text{BMA}(i | Y = y) \sum_{k=0}^1 \text{Pr}(y | \mathcal{M}_k) \text{Pr}(\mathcal{M}_k) \\
\frac{1}{2} = (1 - b_i) \sum_{k=0}^1 \frac{1}{2}(1 - a_k) + b_i \sum_{k=0}^1 \frac{1}{2}a_k\\
1 = \sum_{k=0}^1 (1 - a_k) - b_i \sum_{k=0}^1 (1 - a_k) + b_i \sum_{k=0}^1 a_k \\
1 = 2 - \sum_{k=0}^1 a_k - 2b_i + b_i\sum_{k=0}^1 a_k + b_i \sum_{k=0}^1 a_k \\

0 = (1 - b_0 - 1 + b_1) \sum_{k=0}^1 \frac{1}{2}(1 - a_k) + (b_0 - b_1) \sum_{k=0}^1 \frac{1}{2}a_k\\
0 = -(b_0 - b_1) \sum_{k=0}^1 \frac{1}{2}(1 - a_k) + (b_0 - b_1) \sum_{k=0}^1 \frac{1}{2}a_k\\
