...
for(N in N_to_test){
  for(bp_name in names(bp_correct)) {
     ...
     if(file.exists(cache_file)) {
       dap_power_res <- readRDS(cache_file)
     } else {
       dap_power_res <- 
         dap_power_scenario(bp_name, N_sims = 1000,  probs, N = N, bp_correct[[bp_name]]$prob1_prior)
       saveRDS(dap_power_res, file = cache_file)
     }
     res_correct_list[[length(res_correct_list) + 1]] <- dap_power_res
  }
}
...
# warning("Hack, figure out the problem")
# bp_turtles_bad_norm$prob <- 1 - bp_turtles_bad_norm$prob
...
bp_wrong <- list("Poisson NB - bias" = list(bp = bp_pnb_bias, prob1_prior = 0.5),
                   "Turtles - bad normalization" = list(bp = bp_turtles_bad_norm, prob1_prior = bp_turtles_bad_norm$simulated_value),
                   "Turtles - bad normalization - unpaired" = list(bp = bp_turtles_bad_norm, prob1_prior = bp_turtles_bad_norm$simulated_value, unpaired = TRUE),
...
for(N in N_to_test){
  for(bp_name in names(bp_wrong)) {
     probs <- bp_wrong[[bp_name]]$bp$prob
     unpaired <- !is.null(bp_wrong[[bp_name]]$unpaired) && bp_wrong[[bp_name]]$unpaired
     cache_file <- file.path(cache_dir, paste0("dap_metrics_wrong_", rlang::hash(list(probs, N)), if_else(unpaired, "_unpaired", "")))
     if(file.exists(cache_file)) {
       dap_power_res <- readRDS(cache_file)
     } else {
       dap_power_res <- 
         dap_power_scenario(bp_name, N_sims = 1000,  probs, N = N, bp_wrong[[bp_name]]$prob1_prior, unpaired = unpaired)
       saveRDS(dap_power_res, file = cache_file)
     }
     res_wrong_list[[length(res_wrong_list) + 1]] <- dap_power_res
  }
}
