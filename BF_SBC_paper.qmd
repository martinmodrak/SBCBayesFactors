...
In this paper, we set the discussion about the usefulness of Bayes factors aside, and simply assume we have a scenario where Bayes factors are a useful quantity of interest. An important practical concern is how to compute Bayes factors and/or the marginal
...
Our main contribution is to employ an improved variant simulation-based calibration checking  (SBC, Modrák et al.) and the accompanying theoretical progress for validating Bayes factor computation. We show a theoretical equivalence between SBC for Bayes factors and binary prediction calibration. Using several toy models, we demonstrate how problems in Bayes factor computation not detectable by inspecting data-averaged posterior can easily be detected by SBC. Finally, we turn to real-world problems and validate some uses of bridgesampling over Stan models, compare efficiency of various checks in discovering errors in normalization constants and show that using posterior SBC (cite Vehtari) is necessary when checking Bayes factor computation under improper priors for some model parameters.

Previously, Schad and co-authors have developed a workflow for validating Bayes factor computation inspired by SBC and applied it to several problems (cite workflow, aggregate, biased). Both the theoretical and practical contributions of this paper are partially a reaction to Schad et al.: from the theoretical side, we note that the Schad et al. approach differs importantly from SBC both as originally described (cite Talts) as well as our improved variant. In fact the Schad et al. approach and is better understood as a check based on data-averaged posterior. Therefore the approach of Schad et al. uses simulation results inefficiently and is insensitive to large classes of potential problems. Additionally, due to incorrect implementation at least some of the cases where Schad (biased BF) claim incorrect Bayes factor computations are false positives and we show that the resulting Bayes factors are in fact calibrated to high precision.

We conclude the paper with recommendations for checking Bayes factor computation both when developing new statistical tools as well as a part of Bayesian workflow (following Gelman et al. workflow preprint)
...
or with bimodal distribution and low number of simulations), 
...
We complement the t-test with the test of Gaffke 2005.
...
turtles_post_half_test$prob1_tweaked_prior
```
...
This can be overcome by rejection sampling --- as in any other Bayesian inference, selectively removing datasets based on properties of $y$ does not change the posterior at all. So we remove all datasets where $\text{Avg}(y) < \frac{1}{10}$ or $\text{Avg}(y) > \frac{9}{10}$.
We note that to keep SBC valid those rejections must be done for the whole BMA model, not when simulating each model individually^[The first author of this paper has spent an embarrassing amount of time tracking down the source apparent miscalibration in this model when the problem actually stemmed from rejecting datasets from each submodel separately. This illustrates the importance of trying to understand _why_ a model is calibrated and not blindly reporting SBC results.]. 
While rejections don't impact the posterior, they change the implied prior and since we now no longer have a known prior distribution, we must change the data-averaged posterior check to a two-sample test for equality of means between the true values for the non-rejected datasets. In this example, rejections change the implied prior to $P(\mathcal{M}_1) \simeq `r round(implied_prior_rejection, 2)`$.


For the second check, we focus the simulations to the neighbourhood of the actual data. 
...
as prior for the model index, but to show an alternative, we can also choose $P(M_1) \simeq `round(turtles_post_half_test$prob1_tweaked_prior,2)`$,
...
The authors note that 9 out of 21 investigated Stan models using a truncated distribution neglected the normalizing constant, which would lead to biased Bayes factors obtained with `bridgesampling`, if those models were used to compute them. Here we test how efficiently do our checks discover this problem using the
example from the previous section, but modifying the prior so that $\tau \sim \text{HalfN}(0, 1)$, but we neglect the normalization constant and so the  density is wrong by a factor of 2.
...
One example is the `ttestBF` in BayesFactor package, representing the JZS test, we focus on the one-sample variant. 
...
and matching the model to the simulation code we obtain calibrated Bayes factors. We have run `r format(n_schad_sim, big.mark  = " ")` simulations (some were removed due to problems sampling) and obtained miscalibration p `r format_p(schad$miscalibration_p)`, 95% CI for data-averaged posterior difference from expected mean `r schad_ci_diff`.
...
A practical problem we encountered with checking the methods in `BayesFactors` 
was that the priors in use are described only in research papers and require non-trivial effort to fully trace. In contrast, when `brms` or
a Stan program was used to compute the models the priors are transparently accessible.

Our recommendation for researchers wishing to implement and/or evaluate  tools that compute the Bayes factor where there is no good source of "correct" Bayes factors are to use SBC for the full BMA model accompanied with binary calibration check and data-averaged posterior check. 
...
In our tests, both `bridgesampling` and the `BayesFactors` package provided reliable
Bayes factor estimates in all tests we ran. Bias previously reported by Schad & Vasishth was an artifact
...
### Broader implication and future work
...
A possible future work is improving SBC to match performance of miscalibration testing --- SBC throws away information by random tie-braking. When that information is kept, the variance of the eCDF decreases. However, there is no known way to evaluate the distribution of SBC outcomes under the correct model to make the test more stringent. 
