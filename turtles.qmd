...
nc <- xlim / (1 + xlim)
pdf_df <- data.frame(x = x_vals, y = (1 + x_vals)^-2 / nc)
...
```{r simulateds-chisq}
set.seed(84662)
ds_turtles_m1_chisq <- generate_datasets(SBC_generator_function(sim_turtles, model = 1, N = NULL, C = NULL, sigma2_prior = "chisq"), n_sims = length(ds_turtles_m0))
...
plot_binary_calibration_diff(res_turtles_chisq$stats)
