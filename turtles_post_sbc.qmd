...
gen_globals <- c("r_inv_one_plus_sq", "turtles", "draws_prior_H0", "draws_prior_H1")

sim_turtles_post_sbc <- function(model, N_per_old_clutch, N_per_new_clutch = 5, C_new = 0, sigma2_prior = "default", prior_width = sqrt(10.0), link = "probit") {
  if(model == 0) {
...
res_turtles_post <- compute_SBC(ds_turtles, backend_turtles, 
                           keep_fits = FALSE,
                           cache_mode = "results",
                           cache_location = file.path(cache_dir, "turtles_post.rds"))
```
