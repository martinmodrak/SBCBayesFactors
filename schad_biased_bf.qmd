...
fits_cache_dir <- file.path(cache_dir, "fits_2step_Item")
...
```{r}
backend_2step_H1 <- SBC_backend_cached(
...
backend_2step_H0_1 <- SBC_backend_cached(
  SBC_backend_brms(ysim ~ 1 +      trans + rew_trans + (1 + rew + trans + rew_trans  | subj) + (1 + rew + trans + rew_trans | item),                          family=gaussian(), prior=priors2step, 
                  save_pars = save_pars(all = TRUE),
                  warmup=2000, iter=10000,
            template_data = simdat2step,
...
backend_2step_H0_correct <- SBC_backend_cached(
