...
```{r}
metrics_orig <- calibration_metrics(res_turtles, prob1_prior = as.numeric(ds_turtles$variables[,"model"]))
print(metrics_orig)
```


```{r save-stats}
saveRDS(metrics_orig,
  file.path(cache_dir, paste0("turtles_metrics.rds")))
...
```{r}
bp_orig <- binary_probabilities_from_stats(res_turtles$stats)
good_check_okada(bp_orig$prob, bp_orig$simulated_value, prior_prob1 = mean(as.numeric(ds_turtles$variables[,"model"])))

hist_orig_okada <- compute_bootstrapped_histories(bp_orig, history_length = 500, n_histories = 100, compute_okada_history, min_sim_id = 20, prior_prob1 = mean(as.numeric(ds_turtles$variables[,"model"])))

plot_log_p_histories(hist_orig_okada, "Fractional Good check")
```
...
```{r}
good_check_okada(bp_turtles_norm$prob, bp_turtles_norm$simulated_value, prior_prob1 = mean(bp_turtles_norm$simulated_value))
```
...
```{r}
metrics_norm_bad <- calibration_metrics(res_turtles_norm_bad, prob1_prior = as.numeric(ds_turtles_norm$variables[,"model"]))
print(metrics_norm_bad)
```
...
hist_turtles_norm_bad_okada <- compute_bootstrapped_histories(bp_turtles_norm_bad, history_length = hist_length_turtles_norm_bad, n_histories = n_histories, step = hist_step_turtles_norm_bad,
                                                           compute_okada_history, min_sim_id = 10, prior_prob1 = mean(as.numeric(ds_turtles_norm$variables[,"model"])))
...
plot_log_p_histories(hist_turtles_norm_bad_okada, "Fractional Good check")

save_histories("turtles_norm_bad",
...
set.seed(985243245)
ds_turtles_half <- SBC_datasets_for_bf(ds_turtles_m0_half, ds_turtles_m1_half, prob_H1 = prob1_target_half)
```
...
```{r}
metrics_turtles_post_half <- calibration_metrics(res_turtles_post_half, prob1_prior = prob1_target_half)
metrics_turtles_post_half$prob1_tweaked_prior_half <- prob1_tweaked_prior_half
print(metrics_turtles_post_half)
```



```{r save-stats-post-half}
saveRDS(metrics_turtles_post_half,
  file.path(cache_dir, paste0("turtles_post_half_metrics.rds")))
...
```{r simulateds-post-full}
N_per_old_clutch <- 0
N_per_new_clutch <- 4
C_new <- 25

set.seed(345852)
ds_turtles_post_m0 <- generate_datasets(
  SBC_generator_function(
    sim_turtles_post_sbc,
    data_prior = turtles,
...
ds_turtles_post_m1 <- generate_datasets(
  SBC_generator_function(
    sim_turtles_post_sbc,
    data_prior = turtles,
    draws_prior_H0 = draws_prior_H0,
...
set.seed(68234245)
ds_turtles_post <- SBC_datasets_for_bf(ds_turtles_post_m0, ds_turtles_post_m1, prob_H1 = prob1_target)
```

```{r turtles1-post}
backend_turtles_post <- SBC_backend_bridgesampling(
  SBC_backend_rstan_sample(m_H0, iter = iter, warmup = warmup, init = init),
  SBC_backend_rstan_sample(m_H1, iter = iter, warmup = warmup, init = init),
  prior_prob1 = prob1_tweaked_prior
)
res_turtles_post <- compute_SBC_cache_blocks(ds_turtles_post, backend_turtles_post, 
                           keep_fits = FALSE,
...
```{r}
metrics_turtles_post <- calibration_metrics(res_turtles_post, prob = prob1_target)
print(metrics_turtles_post)
```
