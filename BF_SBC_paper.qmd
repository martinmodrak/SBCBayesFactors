...
SBC for Bayes Factors is in fact the same as binary prediction calibration, i.e. considering the BMA supermodel, $\phi_\text{BMA}$ passes continuous SBC w.r.t. $f: \Theta_\text{BMA} \times Y \to \{0, \ldots K - 1\}, f((i, \theta_i), y) = i$ if and only if $\phi_\text{BMA}$ passes the prediction calibration check.
...
The method of Schad et al. is to check the data-averaged posterior criterion for the model index in the BMA supermodel (i.e. with $f: \Theta_\text{BMA} \to \{0, \ldots K - 1\}$ and $\forall (i, \theta_i) \in \Theta_\text{BMA}: f((i, \theta_i)) = i$).
