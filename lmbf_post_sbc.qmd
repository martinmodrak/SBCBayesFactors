...
sim_lmbf_fixed_data <- function(N) {
  data.frame(id = 1:N,
             ...
             x1 = c("A","A","B","B", sample(c("A", "B"), size = N_prior_data - 4, replace = TRUE)),
             x2 = c("I","J","I","J", sample(c("I", "J"), size = N_prior_data - 4, replace = TRUE)))
}
...
N <- 10
n_sims <- 4000
alpha_sigma_from_draws <- function(draws) {
  function() {
...
ds_lmbf_post_H0 <- generate_datasets(SBC_generator_function(sim_lmbf_fixed, N = 100, formula = y ~ x1, alpha_sigma_prior = alpha_sigma_from_draws(draws_prior_H0)), n_sims = n_sims)
ds_lmbf_post_H1 <- generate_datasets(SBC_generator_function(sim_lmbf_fixed, N = 100, formula = y ~ x1 + x2, alpha_sigma_prior = alpha_sigma_from_draws(draws_prior_H1)), n_sims = n_sims)
```
...
res_post <- compute_SBC(ds_post, backend_post, keep_fits = FALSE,
                            cache_mode = "results",
                            cache_location = file.path(cache_dir, paste0("lmbf_post_", N_for_filename, ".rds")))

plot_ecdf_diff(res_post)
bp_post <- binary_probabilities_from_stats(res_post$stats)
t.test(bp_post$prob, mu = 0.5)
...
ds_lmbf_constant_H0 <- generate_datasets(SBC_generator_function(sim_lmbf_fixed, N = 100, formula = y ~ x1, alpha_sigma_prior = constant_alpha_sigma), n_sims = n_sims)
ds_lmbf_constant_H1 <- generate_datasets(SBC_generator_function(sim_lmbf_fixed, N = 100, formula = y ~ x1 + x2, alpha_sigma_prior = constant_alpha_sigma), n_sims = n_sims)
```
...
res_constant <- compute_SBC(ds_constant, backend_constant, keep_fits = FALSE,
                            cache_mode = "results",
                            cache_location = file.path(cache_dir, paste0("lmbf_post_constant_", N_for_filename, ".rds")))
