...
sqrt_norm_df <- data.frame(x = x_vals, y = 0.5 * x_vals^(-0.5) * 2 * dnorm(sqrt(x_vals), sd = sqrt(2 / (1 - 2/pi))))
ggplot() + aes(x = x ,y = y) + 
  geom_line(data = pdf_df, aes(y = y), color = "blue") + 
  geom_line(data = chisq_df, aes(y = y), color = "red") +
  geom_line(data = sqrt_norm_df, aes(y = y), color = "green") 

ggplot() + aes(x = x ,y = y) + geom_line(data = pdf_df, aes(y = y), color = "blue") + 
  geom_line(data = chisq_df, aes(y = y), color = "red")  +
  geom_line(data = sqrt_norm_df, aes(y = y), color = "green")  + scale_y_log10()
```
  ...
  for(i in 1:200) {
    ...
    if(model == 0) {
      ...
      if(sigma2_prior == "default") {
        ...
        sigma_prior_sd <- sqrt(2 / (1 - 2/pi)) # Match the chi-squared variance
        sigma_clutch <- abs(rnorm(1, sd = sigma_prior_sd))
        
        sigma2_log_lik <- log(sigma_clutch)  +# Jacobian
          log(2) + dnorm(sigma_clutch, sd = sigma_prior_sd, log = TRUE)
      } else {
...
set.seed(54223248)
N_sims <- 1000
ds_turtles_m0 <- generate_datasets(SBC_generator_function(sim_turtles, model = 0, N = N, C = C, future.globals = "turtles"), n_sims = N_sims)
ds_turtles_m1 <- generate_datasets(SBC_generator_function(sim_turtles, model = 1, N = N, C = C, future.globals = "turtles"), n_sims = N_sims)
...
```{r}
# TODO: this needs to change after the update
...
```{r}
# GOOd test?
true_model <- res_turtles$stats %>% filter(variable == "model") %>% pull(simulated_value)
p_H1 <- res_turtles$backend_diagnostics$prob_H1
bf01 <- (1 - p_H1) / p_H1
mean(bf01[true_model == 1])
mean(bf01[true_model == 0])
