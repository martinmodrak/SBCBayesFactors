...
In this case, $\mathbb{Var}(BF_{0,1} | \mathcal{M}_1)$ is undefined. This is admittedly a bit extreme, most models do not feature Cauchy marginal data distributions. So let's assume that for a fixed, known $\mu$ we instead have $\mathcal{M}_1: y \sim \text{N}(\mu, 1)$. Then we have $\mathbb{Var}(BF_{0,1} | \mathcal{M}_1) = \mathbb{Var}(BF_{1,0} | \mathcal{M}_0) = \exp\left(\mu^2\right)$, so e.g. already for $\mu = 2$ we need over 5 000 simulations just to bring the standard error on the expectation below $0.1$. See appendix for full calculation.

The absence of any bound on variance of the estimate which explains the lack of convergence for some cases 
...
### Prediction calibration and ignoring data

Satisfied when data are ignored

### Proof - binary calibration, SBC and data-averaged posterior

We will work with continuous SBC. TODO: copy definitions from Modrák et al.
...
**Definition:** A candidate posterior $\phi$ satistfies binary calibration check w.r.t. 
$f: \Theta \times Y \to \{0,1\}$ if

$$
\forall p: 0\leq p \leq 1:\\
\left( \int_{Y} \text{d}y \int_{\theta} \text{d}y \ \mathbb{I}(f(\theta, y) = 1)  \phi(\theta|y) \pi_\text{marg}(y) \right)  > 0 \implies \text{Pr}\left(f(\theta, y) = 1 | \int_\Theta \: \text{d}\theta \: f(\theta, y)\phi(\theta | y) = p \right) = p 
$$
...
$$
\forall p: \text{Pr}\left(f(\theta, y) = 1 | \right) = p
$$

Proof bin calibration equivalent SBC for binary vars.

Outline:

Explore how $q$ looks when calibrated and not calibrated.
...
When fully continous - use the StackOverflow proof
...
where in the last step, the integral is over the PDF of a normal distribution with mean $2\mu$ and standard deviation $1$.
