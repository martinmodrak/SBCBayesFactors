...
For the data-averaged posterior we use the results from a standard t-test.
For binary prediction calibration, we use a bootstrap-based miscalibration test based on score decomposition described in
[Dimitriadis, Gneiting, Jordan (2021)](https://doi.org/10.1073/pnas.2016191118). 
Specifically, miscalibration reported here is the difference between the actual Brier score of the forecasts and Brier score from a suitably recalibrated forecast which provides a meaningful interpretation to its values.
Following Dimitriadis et al. we calculate bootstrap-based null distribution for miscalibration by repeatedly sampling new observed binary outcomes as if the computed probabilities were correct. This lets us compute a p-value as well as the 95\% quantile of the null distribution.
...
the fewest simulations, see @fig-pnb-bias.
...
and calculate the posterior model probabilities. We then use the fitted parameters to simulate
...
This is quickly picked up by all checks, with binary prediction calibration and data-averaged posterior being the fastest (@fig-turtles-bad-norm). 
...
(plot_log_gamma_histories(hist_turtles_norm_bad$log_gamma) + labs(tag = "A) SBC") + tag_theme) / (
...
binary prediction calibration test with data-averaged posterior and SBC for the model index following very quickly. However
...
simulations (@fig-ranef-post; `r report_success_metrics(ranef_test)`). So we can conclude that
...
derived quantities  (`r report_success_metrics(ttest_post, dap_digits = 4)`). So we conclude that the `ttest` function in fact works well and
...
((plot_ecdf_diff(res_ttest_inv1psquared, variables = "model_sd_m1")  + theme(legend.position = "bottom") + labs(tag = "A) SBC") + tag_theme_calib) | (ttest_inv1psquared$high$reliability_diag  + labs(tag = "B) Calibration - sd(y) ≥ 1") + tag_theme_calib) | 
...
((plot_ecdf_diff(res_ttest_cauchy, variables = "model_sd_m1") + theme(legend.position = "bottom") + labs(tag = "A) SBC") + tag_theme_calib) | (ttest_cauchy$high$reliability_diag  + labs(tag = "B) Calibration - sd(y) ≥ 1") + tag_theme_calib) | 
