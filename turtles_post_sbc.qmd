...
sim_turtles_post_sbc <- function(model, data_prior, draws_prior_H0, draws_prior_H1, N_per_old_clutch, N_per_new_clutch = 5, C_new = 0, sigma2_prior = "default", prior_width = sqrt(10.0), link = "probit") {
  if(model == 0) {
...
if(!file.exists(m0_cache_file)) {
  stanfit_H0_half <- sampling(m_H0,
                     data = list(y = turtles_half$y, x = turtles_half$x, N = nrow(turtles_half), prior_width = sqrt(10.0), link = 0),
                     iter = 15500, warmup = 500, chains = 10, cores = 10, refresh = 4000, seed = 456622342)
...
if(!file.exists(m1_cache_file)) {
  stanfit_H1_half <- sampling(m_H1,
                   data = list(y = turtles_half$y, x = turtles_half$x, N = nrow(turtles_half),
                               C = max(turtles_half$clutch), clutch = turtles_half$clutch, prior_width = sqrt(10.0), link = 0),
                   iter = 15500, warmup = 500, chains = 10, cores = 10, refresh = 4000, seed = 846522143)
