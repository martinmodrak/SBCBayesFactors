...
Stan + `bridgesampling` combination and testing the presence of random effect in TODO interpretation.
We therefore currently believe that for low-stakes statistical workflow involving Bayes factors and standard models, it is relatively safe to rely on those packages without further testing. However, when using a less standard model or when the costs of being wrong are non-trivial, we strongly encourage running at least several hundred simulations of SBC, preferably tailored to the dataset in question with posterior SBC. 
