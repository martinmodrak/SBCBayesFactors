...
```{r}
no_error_index <- res_turtles$backend_diagnostics$bs_error_H0 == 0 & 
...
N <- 80
C <- 10
ds_turtles_m0_more_data <- generate_datasets(SBC_generator_function(sim_turtles, model = 0, N = N, C = C, future.globals = gen_globals), n_sims = N_sims)
ds_turtles_m1_more_data <- generate_datasets(SBC_generator_function(sim_turtles, model = 1, N = N, C = C, future.globals = gen_globals), n_sims = N_sims)
ds_turtles_more_data <- SBC_datasets_for_bf(ds_turtles_m0_more_data, ds_turtles_m1_more_data)

res_turtles_more_data <- compute_SBC(ds_turtles_more_data, backend_turtles, 
                           keep_fits = FALSE,
                           cache_mode = "results",
                           cache_location = file.path(cache_dir, paste0("turtles_N", N, "_C",C,"_", length(ds_turtles_more_data), ".rds")))

```

```{r}
res_turtles_more_data$backend_diagnostics$prob_H1 %>% hist() 
plot_ecdf_diff(res_turtles_more_data, combine_variables = combine_array_elements)
```


```{r}
bp_turtles_more_data <- binary_probabilities_from_stats(res_turtles_more_data$stats)
t.test(bp_turtles_more_data$prob, mu = 0.5)
miscalibration_resampling_p(bp_turtles_more_data$prob, bp_turtles_more_data$simulated_value)
my_reliability_diag(bp_turtles_more_data)
```
