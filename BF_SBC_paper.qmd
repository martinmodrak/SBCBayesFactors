...
An advantage of using SBC instead of just binary calibration is that we can extend the check --- we can
...
1) Many packages use improper priors, which we cannot simulate from.
...
All three problems can be overcome with *posterior SBC* first described in
...
we assume the data space partitions as $Y = Y_1 \times Y_2$, then given a distribution $g$ over
$Y_1$ we can construct a model $\bar\pi$ such that:

$$
\begin{aligned}
\bar\pi_\text{prior}(\theta) &=  \int_{Y_1} \text{d}y_1 \: g(y_1)  \pi_\text{post}(\theta | y_1) \\
\bar\pi_\text{post}(\theta | y_2) &= \pi_\text{post}(\theta | (y_1, y_2)) \\
\end{aligned}
$$

We can then simulate from $\bar\pi_\text{prior}$ by drawing $y_1$ from $g$, fitting $\pi$ to $y_1$ and 
taking a draw from $\pi_\text{post}(\theta|y_1)$. We may then use this parameter draw
to simulate $y_2$ from $\pi_\text{obs}$ and fit $\pi$ to the whole
dataset $(y_1,y_2)$. 
The situation simplifies if $g$ is degenerate distribution and we effectively consider just a single fixed $y_1$.

To check computation for models with improper priors, we need to choose $Y_1$ to cover the
smallest dataset size such that $\pi_\text{post}(\theta | y_1)$ is proper for all $y_1 \in Y_1$ with non-zero density under $g$. 
To provide a better prior, we take $g$ as representing prior beliefs about the data (which tend to be easier to elicit than
...
Another important factor in choosing $g$ thus needs to be that it does not produce too extreme implied prior probability for any 
of the models under consideration.

Alternatively, if we use a fixed $y_1$, we can tweak the
...
and intercept results in perfect calibration in all methods even after running `r format(n_ranef_sims, big.mark = " ")`
...
The example we will use the `ttestBF` in BayesFactor package, representing the JZS test.
There is an improper Jeffrey's prior on variance
...
ttest_ci_diff <-  format_t_test_diff(ranef_test$t)
...
and matching the model to the simulation code we obtain calibrated Bayes factors.
