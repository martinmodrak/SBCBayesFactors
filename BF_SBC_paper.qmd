...
There are many approaches to compute Bayes factors. Savage-Dickey TODO. 
A flexible method is bridgesampling, which can work with any set of samples. Specific families of models have also custom integration methods developed (cite lmBF) TODO a bit more review.
...
Previously, Schad and co-authors have developed a workflow for validating Bayes factor computation inspired by SBC and applied it to several problems (cite workflow, aggregate, biased). Both the theoretical and practical contributions of this paper are partially a reaction to Schad et al.: from the theoretical side, we note that the Schad et al. approach differs importantly from SBC both as originally described (cite Talts) as well as our improved variant. In fact the Schad et al. approach is better understood as a check based on data-averaged posterior. We show that data-averaged posterior checks typically use simulation results inefficiently and are insensitive to large classes of potential problems. Additionally, due to incorrect implementation at least some of the cases where Schad (biased BF) claim incorrect Bayes factor computations are false positives and we show that the resulting Bayes factors are in fact calibrated to high precision (full results reported in Appendix).
...
The posterior distribution $\pi_\text{BMA}(i \mid y)$ of the model index $i$ is then fully determined by the
...
taking draws from $\pi_\text{post}(\theta\midy_1)$. We may then use this parameter draw
to simulate $y_2$ from $\pi_\text{obs}$ and fit $\pi$ to the whole
dataset $(y_1,y_2)$. 
This setup can be generalized further by replacing the single fixed $y_1$ with a distribution $g$ over possible $y_1$, so we have $\bar\pi_\text{prior}(\theta) \int_{Y_1} \text{d}y_1 \: g(y_1)  \pi_\text{post}(\theta \mid y_1)$. The overall idea bears is similar, but distinct to fractional Bayes factors (https://doi.org/10.1111/j.2517-6161.1995.tb02017.x)
...
the parameters of both models, but we also need the new prior for the model index $\bar\pi_\text{BMA}(i\midy_1)$. 
...
important for a calibration check. 
...
bounded distribution of which we've found the test of Gaffke 2005 to work best. The test is more
...
Where we assume uniform prior over models for simplicity, but all of examples can be easily generalized to non-uniform priors.
...
For prediction calibration, we use the p-value from the bootstrap-based miscalibration test described in
...
tag_theme <- theme(plot.tag.location = "plot", plot.tag.position = c(0, 1.01), plot.tag = element_text(hjust = 0))
...
plot_log_p_histories(hist_binary_correct$miscalibration, "Calibration") + scale_x_correct + labs(tag = "B) Calibration") + tag_theme | 
plot_log_p_histories(hist_binary_correct$schad, "DAP - Schad et al.") + scale_x_correct + labs(tag = "C) DAP") + tag_theme)   + plot_layout(widths = c(2,1,1), axis_titles = "collect_x")
```

<!-- Redo tags in other plots -->

However, if we compute the Bayes factors as if the probabilities were flipped
(i.e. computing the inverse of the true Bayes factor), the problem is immediately flagged by all tests except for data-averaged posterior (@fig-binary-flipped). This model is however somewhat artificial - only two possible Bayes factor values, let us
...
plot_log_p_histories(hist_binary_flip$miscalibration, "Calibration") | 
...
In our experiments we used $N = 25$ as this makes the observed posterior probabilities of models values cover the full $[0,1]$ interval. In that setting, the correct BF is just the likelihood ratio of the
two models.
...
detected by binary calibration or data-averaged posterior, but SBC will detect it with suitable derived
...
plot_log_p_histories(hist_pnb_vari$miscalibration, "Calibration") + 
...
plot_log_p_histories(hist_pnb_bias$miscalibration, "Calibration") + 
...
```{r}
turtles_test <- load_precomputed_file(here::here("cache/turtles_stats.rds"), "turtles.qmd")
turtles_ci_diff <- format_t_test_diff(turtles_test$t)
implied_prior_rejection <- turtles_test$t$estimate[2]
n_sims_turtles <- 2000
warning("Get n_sims_turtles from results")

turtles_post_half_test <- load_precomputed_file(here::here("cache/turtles_post_half_stats.rds"), "turtles.qmd")
turtles_post_half_ci_diff <- format_t_test_diff(turtles_post_half_test$t)
n_sims_turtles_post_half <- turtles_post_half_test$t$parameter + 1
```
...
This can be overcome by rejection sampling --- as in any other Bayesian inference problem, selectively removing datasets based on properties of $y$ does not change the posterior at all as long as we always take a new draw from the prior for the next dataset. So we remove all datasets (and the associated prior draws) where $\text{Avg}(y) < \frac{1}{10}$ or $\text{Avg}(y) > \frac{9}{10}$.
We note that to keep SBC valid those rejections must be done for the whole BMA model, not when simulating each model individually^[The first author of this paper has spent an embarrassing amount of time tracking down the source an apparent miscalibration in this model when the problem actually stemmed from rejecting datasets from each submodel separately. This illustrates the importance of trying to understand _why_ a model is calibrated before claiming there is a problem in computations.]. 
While rejections don't impact the posterior, they change the implied prior and since we now no longer have a known prior distribution, we must change the data-averaged posterior check to a two-sample test for equality of means between the true values for the non-rejected datasets. This test needs to allow for different variance in both samples as one is binary and the other is continuous. In this example, rejections change the implied prior to $P(\mathcal{M}_1) \simeq `r round(implied_prior_rejection, 2)`$ and after running `r format(n_sims_turtles, big.mark = " ")` we obtain gamma statistics in the range TODO (95% CI around the null = TODO), binary miscalibration of TODO (95% CI around the null = TODO) and 95% CI for the difference between prior and data-averaged posterior mean for the model index of `r turtles_ci_diff`.
...
as prior for the model index, but to show an alternative, we can also choose $P(M_1) \simeq `r round(turtles_post_half_test$prob1_tweaked_prior,2)`$,
...
After running `r format(n_sims_turtles_post_half, big.mark = " ")` simulations we obtain gamma statistics in the range TODO (95% CI around the null = TODO), binary miscalibration of TODO (95% CI around the null = TODO) and 95% CI for the difference between prior and data-averaged posterior mean for the model index of `r turtles_ci_diff`.
...
This is quickly picked up by all checks, with binary calibration and data-averaged posterior being the fastest. TODO - show  evals
...
plot_log_p_histories(hist_ranef_constant$miscalibration, "Calibration") + 
...
In all tests we ran, as well as those tested by Schad & Vasishth both `bridgesampling` and the `BayesFactor` package provided reliable
Bayes factor estimates. Bias previously reported by Schad & Vasishth was an artifact
of invalid testing procedure. 
...
All of the methods we discuss here and in our recommendations are available for easy use with the `SBC` R package.
...
[Schad & Vasishth (2024)](https://arxiv.org/pdf/2406.08022v1) apply their
method based on data-averaged posterior to show that in some settings Bayes factors from
popular packages are biased. In particular, they claim there is bias in "2x2 repeated 
measures design with crossed random effects for subjects and items" using both the `BayesFactor` package
as well as `bridgesampling` with `brms` fits. 
...
the same problems appear in the results for the `BayesFactor` package as the reported 
...
and matching the model to the simulation code we obtain calibrated Bayes factors. We have run `r format(n_schad_sim, big.mark  = " ")` simulations (some were removed due to problems sampling) and obtained miscalibration `r format_p(schad$miscalibration_p)` (95% CI under the null = TODO), 95% CI for data-averaged posterior difference from expected mean `r schad_ci_diff` and no problem in SBC for any quantity.
...
used in the methods from `BayesFactor` package by taking a fixed value for the parameter, which --- as we have shown ---
...
We are in contact with the preprint authors and we are collaborating on an updated version addressing the issues.
