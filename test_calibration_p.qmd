...
worst_case <- function(worst_n= 10) {
  if(vs < vt - sqrt(vt^2 - vt)) {
    sample(c(vs,vt), size = worst_n, prob = c(vt - 1, 1 - vs) , replace = TRUE)
  } else {
    sample(c(0,vs,vt), size = worst_n, prob = c(1 - vt/(vs*(2*vt - vs)), (vt-vs)/(vs*(2*vt - vs)), 1 / (2*vt - vs) ), replace = TRUE)
  }
  
}


#ps <- test_DAP_single(worst_case, n_rep = 1000, func = \(x) ml_dap_p(x, mu = 1, alternative = "less"))
