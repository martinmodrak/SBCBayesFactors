...
$\pi^i_\text{marg}, \pi^i_\text{prior}, \pi^i_\text{obs}$ (without argument) to denote the distributions
...
\forall p,k: 0\leq p \leq 1,  k \in \{0, \dots, K - 1\}:\\
 \text{Pr}(\mathcal{M}_k | y \in \text{Pred}^\phi_{k, p} ) = p 
$$
...
This property can be empirically checked via simulations. For each $s \in {1, \dots, S}$ 
...
distribution to obtain $y_s \sim \pi^{i_s}_\text{marg}$ and compute $\phi_\text{BMA}(j | y_s)$ for
$j \in \{0, \dots, K - 1\}$.
...
original prior probability, to get uniform distribution in the implied prior, i.e. choose $Pr(\mathcal{M}_k)$ such that $\bar\pi_\text{BMA}(i|y_1) = \frac{1}{K}$.
...
For the data-averaged posterior we use the standard t-test and --- to compare to
...
$$
Y = \{0,1\}, \: \text{Pr}(y = 1 | \mathcal{M}_0) = \frac{1}{5}, \: \text{Pr}(y = 1 | \mathcal{M}_1) = \frac{4}{5}\\
...
plot_log_p_histories(hist_binary_correct$miscalibration, "Calibration") + scale_x_correct | 
...
which results in $P(M_1|y_1) = \frac{1}{2}$ and so our simulations will be more balanced --- this new $P(M_1)$
...
binary scalibration test with data-averaged posterior and SBC for the model index following very quickly. However
...
<!-- $$ -->
<!-- q_{\phi,f}(x|y) :=  -->
<!-- \int_\Theta \mathrm{d} \tilde\theta \: \pi_\text{post}(\tilde\theta | y) \mathrm{Pr}\left(C_{\phi,f} \left(f \left(\left.\tilde\theta, y \right) \right| y \right) - U D_{\phi,f} \left(f \left(\left.\tilde\theta, y \right) \right| y \right) \leq x\right), -->
<!-- $$ -->
...
<!-- $$ -->
<!-- \mathrm{Pr}\left(C_{\phi,f} \left(f \left(\left.\tilde\theta, y \right) \right| y \right) - U D_{\phi,f} \left(f \left(\left.\tilde\theta, y \right) \right| y \right) \leq x \right) \\ -->
<!-- =\begin{cases} -->
<!-- \mathrm{Pr}\left((1 - U)D_{\phi, f}(0 | y) \leq x\right) & f(\tilde\theta, y) = 0 \\ -->
<!-- \mathrm{Pr}\left(1 - UD_{\phi, f}(1 | y) \leq x\right) & f(\tilde\theta, y) = 1 \\ -->
<!-- \end{cases} \\ -->
<!-- =\begin{cases} -->
...
$$
F_i(x) = \mathrm{Pr}(f(\theta, y) \leq x | f(\tilde\theta,y) = i)
$$
...
We now define the CDFs $F^R_i(x) = \mathrm{Pr}(R \leq x | M = i)$
...
$$
We now define the cCDFs $C^R_i(x) = \mathrm{Pr}(R \geq x | M = i)$
