...
The 'squared' variant based on https://github.com/richarddmorey/BayesFactor/issues/156
it appears it is the only one which results in correct calibration :-O

```{r}
variant <- "_squared"
#variant <- ""
#variant <- "_nojacob"
prior_model_H0 <- stan_model(paste0("stan/JZS_ttest_H0", variant, ".stan"))
prior_model_H1 <- stan_model(paste0("stan/JZS_ttest_H1", variant, ".stan"))
```
...
res_prior_H1 <- sampling(prior_model_H1, data = prior_data_stan, iter = 41000, warmup = 1000, thin = 5, cores = 1)
res_prior_H0 <- sampling(prior_model_H0, data = prior_data_stan, iter = 41000, warmup = 1000, thin = 5, cores = 1)
...
```{r}
dquants_ttest <- derived_quantities(pseudo_log_lik = sum(dnorm(y, mean = ifelse(model == 0, 0, mu), sd = sd(y), log = TRUE)))
```
...
ds_ttest_no_sigma$variables <- posterior::subset_draws(ds_ttest$variables, variable = "sigma", exclude = TRUE) 

res <- compute_SBC(ds_ttest_no_sigma, backend, keep_fits = FALSE, globals = ttest_globals,
                   dquants = dquants_ttest,
                   cache_mode = "results",
                   cache_location = file.path(cache_dir, paste0("ttest", variant, ".rds")))
```

```{r}
ddd <- SBC_fit(backend, ds_ttest$generated[[1]], 1)

ddd$bf
attributes(ddd$bf@bayesFactor)
```
...
```{r}
set.seed(265452)
ds_fixed <- generate_datasets(SBC_generator_function(sim_jzs_sigma, N = 5, sigma_generator = 1, r_scale = r_scale), n_sims = 1000)
```
...
set.seed(5422223)
ds_inv1psquared <- generate_datasets(SBC_generator_function(sim_jzs_sigma, N = 5, sigma_generator = r_inv1psquared, r_scale = r_scale), n_sims = 1000)
```
...
```{r}
set.seed(235485)
ds_cauchy <- generate_datasets(SBC_generator_function(sim_jzs_sigma, N = 5, sigma_generator = \() abs(rcauchy(1)), r_scale = r_scale), n_sims = 1000)
```
...
if(include_bs) {
  ...
  res_bs <- compute_SBC(ds_ttest[1:100], backend_bs, keep_fits = FALSE,
                     ...
                     cache_mode = "results",
                     cache_location = file.path(cache_dir, paste0("ttest_bs", variant, ".rds")))
}
```
