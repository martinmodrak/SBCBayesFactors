...
set.seed(32215252)
n_sims <- 200
ds_2step_H0 <- generate_datasets(SBC_generator_function(simulate_2step, model = 0), n_sims = n_sims)
ds_2step_H1 <- generate_datasets(SBC_generator_function(simulate_2step, model = 1), n_sims = n_sims)
```


```{r}
backend_2step_H1 <- SBC_backend_brms(ysim ~ 1 + rew + trans + rew_trans + (1 + rew + trans + rew_trans | subj) + (1 + rew + trans + rew_trans | item),                   family=gaussian(), prior=priors2step, 
                  save_pars = save_pars(all = TRUE),
                  warmup=2000, iter=10000,
            template_data = simdat2step,
            out_stan_file = file.path("cache/2step_H1.stan"))


backend_2step_H0_1 <- SBC_backend_brms(ysim ~ 1 +      trans + rew_trans + (1 + rew + trans + rew_trans  | subj) + (1 + rew + trans + rew_trans | item),                          family=gaussian(), prior=priors2step, 
                  save_pars = save_pars(all = TRUE),
                  warmup=2000, iter=10000,
            template_data = simdat2step,
            out_stan_file = file.path("cache/2step_H0_1.stan"))


backend_2step_H0_correct <- SBC_backend_brms(ysim ~ 1 +      (1 + rew + trans + rew_trans  | subj) + (1 + rew + trans + rew_trans | item),                          family=gaussian(), prior=priors2step[-2,], 
                  save_pars = save_pars(all = TRUE),
                  warmup=2000, iter=10000,
            template_data = simdat2step,
            out_stan_file = file.path("cache/2step_H0_correct.stan"))
```


```{r}
ds_2step <- SBC_datasets_for_bf(ds_2step_H0, ds_2step_H1)
backend_2step <- SBC_backend_bridgesampling(backend_2step_H0_correct, backend_2step_H1)
...
res_2step <- compute_SBC(
  ...
  keep_fits = FALSE,
  cache_mode = "results",
  cache_location = file.path(cache_dir, "SBC_2step.rds"))
...
plot_binary_calibration_diff(res_2step$stats)
calibrationband::calibration_bands(round(res_2step$backend_diagnostics$prob_H1, 8), res_2step$stats %>% filter(variable == "model") %>% pull(simulated_value) %>% as.logical())
```
