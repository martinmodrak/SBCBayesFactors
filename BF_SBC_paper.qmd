...
good_sims_cauchy <- run_good_sims(good_sims, good_runs, m0 = "cauchy", step = good_step, start = good_start)
plot_good_cauchy <- plot_good_example(good_sims_cauchy, linealpha = good_linealpha)

good_sims_norm2 <- run_good_sims(good_sims, good_runs, m0 = 2, step = good_step, start = good_start)
plot_good_norm2 <- plot_good_example(good_sims_norm2, linealpha = good_linealpha)

(plot_good_cauchy | plot_good_norm2) + plot_layout(axis_titles = "collect") + plot_annotation(tag_levels = "A")
  
}

```

```{r fig-good-okada-cauchy}
okada_plots <- function(m0, m1 = 0, ylim = NULL) {
  okada_ci_runs <- 50
  bf01_given_h0_sims <- run_good_sims(good_sims, okada_ci_runs, m0 = m0, m1 = m1, step = good_step, start = good_start, moment = -0.5, numerator = 0, simulate_from = 0)


  bf01_given_h1_sims <- run_good_sims(good_sims, okada_ci_runs, m0 = m0, m1 = m1, step = good_step, start = good_start, moment = 0.5, numerator = 0, simulate_from = 1)


  t_test_p <- numeric(okada_ci_runs)
  t_test_low <- numeric(okada_ci_runs)
  t_test_high <- numeric(okada_ci_runs)
  t_test_est <- numeric(okada_ci_runs)
  t_stat <- numeric(okada_ci_runs)
  t_df <- numeric(okada_ci_runs)
  
  raw_bf1 <- attr(bf01_given_h0_sims, "raw_bf_moment")
  raw_bf2 <- attr(bf01_given_h1_sims, "raw_bf_moment")
  for(i in 1:okada_ci_runs) {
    t_res <- t.test(raw_bf1[,i], raw_bf2[,i])
    t_test_p[i] <- t_res$p.value
    t_test_est[i] <- -diff(t_res$estimate)
    t_test_low[i] <- t_res$conf.int[1]
    t_test_high[i] <- t_res$conf.int[2]
    t_stat[i] <- t_res$statistic
    t_df[i] <- t_res$parameter
  }

  # diff_test <- data.frame(id = 1:okada_ci_runs, low = t_test_low, est = t_test_est, high = t_test_high) |>
  #   ggplot(aes(x = id, y = est, ymin = low, ymax = high)) + geom_hline(yintercept = 0, color = "darkblue") + geom_pointrange(fatten = 0.8) + scale_x_discrete("Run") + scale_y_continuous("Estimated difference")

  min_df <- min(t_df)
  # diff_test <- data.frame(id = 1:okada_ci_runs, t = t_stat) |> 
  #   ggplot(aes(x = id, y = t_stat)) + 
  #   geom_rect(xmin = 1, xmax = okada_ci_runs, ymin = qt(0.0005, df = min_df), ymax = qt(0.9995, df = min_df), fill = "lightblue") +     
  #   geom_hline(yintercept = 0, color = "darkblue") + 
  #   geom_point() + scale_x_discrete("Run") + scale_y_continuous("Welch's t-statistic") 

  alpha <- 0.05
  diff_test <- data.frame(id = 1:okada_ci_runs, t = t_stat) |> 
    ggplot(aes(x = t_stat)) + 
    geom_rect(ymin = -Inf, ymax = +Inf, xmin = qt(alpha / 2, df = min_df), xmax = qt(1 - alpha / 2, df = min_df), fill = "lightblue") +     
    geom_histogram(binwidth = 0.1) +
    scale_x_continuous("Welch's t-statistic") 
  
  diff_test
  bf01_given_h0 <- plot_good_example(bf01_given_h0_sims[,1 : good_runs], linealpha = good_linealpha, correct_value = NULL, ylim = ylim)
  bf01_given_h1 <- plot_good_example(bf01_given_h1_sims[,1 : good_runs], linealpha = good_linealpha, correct_value = NULL, ylim = ylim)
    
  ((bf01_given_h0 + bf01_given_h1) / diff_test) + plot_annotation(tag_levels = "A") + plot_layout(axes = "collect")
...
okada_plots("cauchy", ylim = c(0.86, 0.95))

```

```{r fig-good-okada-norm2}
set.seed(6542224)
okada_plots(2, ylim = c(0.53, 0.68))
okada_plots(10)
```
```{r}
okada_plots(m0 = 0, m1 = "cauchy")
```
...
#### Fractional Good check

The relationship between the probability and fractional bf is mostlly close to linear, explaining the similar performance??? Maybe???

```{r}
x <- seq(0.001,0.999, length.out = 1000); plot(x, sqrt((1 - x) / x))
x <- seq(0.2,0.999, length.out = 1000); plot(x, sqrt((1 - x) / x))
x <- seq(0.001,0.999, length.out = 1000); plot(x, ((1 - x) / x)^-0.5)
x <- seq(0.001,0.2, length.out = 1000); plot(x, ((1 - x) / x)^-0.5)
```
