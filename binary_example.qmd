...
hist_correct_okada <- compute_bootstrapped_histories(bp_correct, history_length = hist_length_correct, n_histories = n_histories, step = hist_step_correct,
...
plot_log_p_histories(hist_correct_okada, "Okada Good check")
...
```{r}
hist_flip <- compute_bootstrapped_histories(res_bin_flip$stats, history_length = 50, n_histories = n_histories,
                                            compute_log_gamma_history)

bp_flip <- binary_probabilities_from_stats(res_bin_flip$stats)


hist_flip_ttest <- compute_bootstrapped_histories(bp_flip, history_length = 50, n_histories = n_histories,
                                            compute_ttest_history, min_sim_id = 10)

hist_flip_miscalib <- compute_bootstrapped_histories(bp_flip, history_length = 50, n_histories = n_histories,
                                            compute_miscalibration_history)


hist_flip_brier <- compute_bootstrapped_histories(bp_flip, history_length = 50, n_histories = n_histories,
                                            compute_brier_history)


hist_flip_okada <- compute_bootstrapped_histories(bp_flip, history_length = 50, n_histories = n_histories, min_sim_id = 20,
...
plot_log_p_histories(hist_flip_okada, "Okada Good Check")
...
Does the Good check (direct vs. fractional) detect flips? Standard yes, fractional NO.


```{r}
good_check(bp_flip$prob, bp_flip$simulated_value)
good_check_okada(bp_flip$prob, bp_flip$simulated_value)
```

Do I get something for DAP by conditioning on the true model? Not really

```{r}
t.test(bp_flip$prob[bp_flip$simulated_value == 0], 1 - bp_flip$prob[bp_flip$simulated_value == 1])
t.test(bp_correct$prob[bp_correct$simulated_value == 0], 1 - bp_correct$prob[bp_correct$simulated_value == 1])
```


```{r}
hist_flip2 <- compute_bootstrapped_histories(res_bin_flip$stats, history_length = 500, n_histories = n_histories,
                                            compute_log_gamma_history)
...
```{r}
hist_prior <- compute_bootstrapped_histories(res_bin_prior$stats, history_length = 50, n_histories = n_histories,
                                            compute_log_gamma_history)
...
hist_small <- compute_bootstrapped_histories(res_bin_small$stats, history_length = hist_length_small, n_histories = n_histories,
                                             step = 50,
                                            compute_log_gamma_history)
...
Note - the fractionl Good check here is hampered by a lot of NAs from purely constant BFs.

```{r}
hist_bad <- compute_bootstrapped_histories(res_bin_bad$stats, history_length = 100, n_histories = n_histories,
                                            compute_log_gamma_history, min_sim_id = 5)



hist_bad_ttest <- compute_bootstrapped_histories(binary_probabilities_from_stats(res_bin_bad$stats) , history_length = 100, n_histories = n_histories,
                                            compute_ttest_history, min_sim_id = 5)


hist_bad_brier <- compute_bootstrapped_histories(binary_probabilities_from_stats(res_bin_bad$stats) , history_length = 100, n_histories = n_histories,
                                            compute_brier_history, min_sim_id = 5)

hist_bad_miscalibration <- compute_bootstrapped_histories(binary_probabilities_from_stats(res_bin_bad$stats) , history_length = 100, n_histories = n_histories,
                                            compute_miscalibration_history, min_sim_id = 10)


hist_bad_okada <- compute_bootstrapped_histories(binary_probabilities_from_stats(res_bin_bad$stats) , history_length = 100, n_histories = n_histories,
                                            compute_okada_history, min_sim_id = 20)
...
plot_log_p_histories(hist_bad_okada, "Fractional Good check")
