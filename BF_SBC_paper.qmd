...
Call Paul 24.07
Artificial example for constant prior:
- flat prior on shared coefficient
...
Data-averaged posterior - BF for _some_ model, expalin that it means unrelated, have an example.

Practical example - close to zero probability for the correct model. (maybe weaker, "on a more theoretical note")

Vertical line for 80% power in the plots.
...
Let us start with the case that the the posterior probabilities are all $\frac{1}{2}$ --- we know that neither data-averaged posterior, nor miscalibration can detect this in principle (and that holds in the simulations), but data-dependent quantities typically discover the problem reasonably quickly.
...
Now choosing the overall intercept as fixed constant in SBC results in apparent miscalibration, but that is a false positive. Using posterior SBC results in perfectly calibrated

The previous example is the simplest setting where we obtain strong miscalibration due to improper priors for shared parameters, however, the problem subtly affects even much simpler settings.
...
However, when we use posterior SBC, the computed Bayes factors pass all checks to high precision.
...
We have also shown that the choice priors for shared parameters like the intercept or standard deviation (which is often given little thought) can have non-negligible effect on Bayes factors, further highlighting the brittleness of Bayes factors.
