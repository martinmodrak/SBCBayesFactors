...
- Real example: Bad vectorization/normalization (based on paper)
  - Do we know what the "usual issues" are? Bad normalization constants probably fail DAP quickly.
...
Now choosing the overall intercept as fixed constant results in apparent miscalibration for both the model index and all the individual random effect parameters (see @fig-ranef-constant). In this case the problem is the most quickly picked up by the miscalibration test with DAP and SBC for the model index following very quickly. However this is a false positive. Employing posterior SBC to produce a proper prior for the mean and intercept results in perfect calibration in all methods even after running 10 000 simulations. So we can conclude that the BF calculation in the BayesFactor package works well for this case.
...
hist_ranef_constant <- load_histories("ranef_constant")
...
The previous example is the simplest setting we discovered where we obtain strong miscalibration due to improper priors for shared parameters, however, the problem subtly affects even much simpler settings.
...
Unfortunately, their work has multiple problems making their conclusions suspect: they do not simulate from the correct prior for multiple parameters (not just those with improper priors) and they focus on DAP (which we know is weak)
