...
hist_correct_schad_bf <- compute_bootstrapped_histories(bp_correct, history_length = hist_length_correct, n_histories = n_histories, step = hist_step_correct,
...
plot_log_p_histories(hist_correct_schad_bf, "Schad BF")
plot_log_p_histories(hist_correct_miscalibration, "Miscalibration resampling")

```

```{r}
compute_bootstrapped_histories(bp_correct, history_length = hist_length_correct, n_histories = 10, step = hist_step_correct,
                                                           compute_miscalibration_history, min_sim_id = 10, cache = FALSE)
```


```{r}
save_histories("bin_correct", gamma = hist_correct, schad = hist_correct_schad, miscalibration = hist_correct_miscalibration)
```
