...
We perform two checks --- for one we use standard SBC and $N = 70, G = 10$ (this keeps the distribuion of observed BFs reasonably wide) and simulate the $x_i \sim N(0, \frac{1}{3})$.
...
While rejections don't impact the posterior, they change the implied prior and since we now no longer have a known prior distribution, we must change the data-averaged posterior check to a two-sample test for equality of means between the true values for the non-rejected datasets. In this example, this changes the implied prior to TODO.
...
as prior for the model index, but to show an alternative, we can also choose $P(M_1) = $ (TODO value),
