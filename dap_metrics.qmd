...
for(N in N_to_test){
  for(bp_name in names(bp_wrong)) {
     ...
     cache_file <- file.path(cache_dir, paste0("dap_metrics_wrong_", rlang::hash(list(probs, N))))
     if(file.exists(cache_file)) {
       dap_power_res <- readRDS(cache_file)
     } else {
       dap_power_res <- 
         dap_power_scenario(bp_name, N_sims = 1000,  probs, N = N, bp_wrong[[bp_name]]$prob1_prior)
       saveRDS(dap_power_res, file = cache_file)
     }
