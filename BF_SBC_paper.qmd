...
model selection and hypothesis testing in the Bayesian paradigm --- this is despite some criticism (TODO reivew + cite). Possibly its main appeal is the close relationship between the Bayes factor and the posterior probability that a hypothesis is true.
...
There are many approaches to compute Bayes factors. Savage-Dickey. 
...
Our main contribution is to employ an improved variant simulation-based calibration checking  (SBC, Modrák et al.) and the accompanying theoretical progress for validating Bayes factor computation. We show a theoretical equivalence between SBC for Bayes factors and binary prediction calibration. Using several toy models, we demonstrate how problems in Bayes factor computation not detectable by inspecting data-averaged posterior can easily be detected by SBC. Finally, we turn to real-world problems and demonstrate cases where bridgesampling over Stan models can have noticeable bias, we show that using posterior SBC (cite Vehtari) is necessary when checking Bayes factor computation under improper priors for some model parameters,  and compare efficiency of various checks in discovering errors in normalization constants. 

Previously, Schad and co-authors have developed a workflow for validating Bayes factor computation inspired by SBC and applied it to several problems (cite workflow, aggregate, biased). This paper is partially a reaction to Schad et al.:  we note that the Schad et al. approach differs importantly from SBC both as originally described (cite Talts) as well as its more recent variants. In fact the Schad et al. approach and is better understood as a check based on data-averaged posterior. We show that the approach of Schad et al. uses simulation results inefficiently and is insensitive to large classes of potential problems. Additionally, due to incorrect implementation at least some of the cases where Schad (biased BF) claim incorrect Bayes factor computations are false positives and we show that the resulting Bayes factors are in fact calibrated to high precision.
...
We perform two checks --- for one we use standard SBC $N = 80, G = 10$ and simulate the $x_i \sim N(0, \frac{1}{3})$.
...
To this end, we pick half of the groups and fit both models to this smaller dataset,
and calculate the Bayes factor (TODO result). We could use the implied model probability 
...
is then used when converting the computed Bayes factor into posterior model probability.
...
The authors note that 9 out of 21 investigated Stan models using a truncated distribution neglected the normalizing constant, leading to possibly biased Bayes factors. Here we test how efficiently do our checks discover this problem using the
